#!/usr/bin/env python
"""
Generic searchers and optimizers.
The way to use this code is to subclass Problem to create a class of problems,
then create problem instances and solve them with calls to the various search
functions.
Original code from AIMA 2nd edition Python code.
"""

from utils import *
import math, random, sys, time, bisect, string, pdb
...
class Node(object):
    ...
    the same state.  Also includes the the total path_cost (also known as g)
    to reach the node.  Other functions may add an f and h value; see
    best_first_graph_search and astar_search for an explanation of how the f and h
    values are handled. You will not need to subclass this class."""

    def __init__(self, state, parent=None, value=0, path_cost=0):
        "Create a search tree Node, derived from a parent by an action."
        update(self, state=state, parent=parent,
               value=value, path_cost=path_cost, depth=0)
        if parent:
    ...
    def expand(self, problem):
        ...
        return (Node(next, self,
                     path_cost=problem.path_cost(self.path_cost, self.state, next))
...
def best_first_graph_search(problem, f):
    ...
    f = memoize(f, 'f')
    return graph_search(problem, PriorityQueue(order=min, key=f))
